...
class StoreKioskController extends AbstractController
{
    ...
    /** Mint a kiosk session while staff still have a valid login. */
    ...
    /**
     * Place a pay-at-counter kiosk order. Authorized by the kiosk session
     * token so walk-ups keep shopping after the staff JWT expires.
     */
    ...
    /**
     * Exit is gated by the store exit code only — no staff JWT required, so a
     * signed-out kiosk terminal is never trapped behind /login.
     */
