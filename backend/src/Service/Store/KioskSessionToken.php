 ...
 * expiry so customers can keep shopping without a session-expired prompt;
 * invalidated when staff exit (store epoch bump) or the token TTL elapses.
 */
final readonly class KioskSessionToken
{
    ...
    public function issue(Store $store): string
    {
        ...
        $payload = [
            ...
            'storeSlug' => $slug,
            'epoch' => $store->getKioskSessionEpoch(),
    ...
    public function verify(Store $store, string $token): bool
    {
        ...
        if ((string) ($payload['storeSlug'] ?? '') !== (string) $store->getSlug()) {
            return false;
        }

        return (int) ($payload['epoch'] ?? -1) === $store->getKioskSessionEpoch();
    }
